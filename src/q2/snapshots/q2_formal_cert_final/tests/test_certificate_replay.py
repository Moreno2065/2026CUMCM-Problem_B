...
def test_full_certificate_replay_passes():
    ...
    assert res.gap <= 0.01
    # the shipped certificate must agree with the fresh replay
    cert = _load_certificate()
    assert res.status == cert["status"]
    L_ship, U_ship = float(cert["lower_bound_m"]), float(cert["upper_bound_m"])
    assert res.lower_bound <= L_ship + 1e-6
    assert res.upper_bound >= U_ship - 1e-6
    # replayed replay must not contradict the frozen bounds (s.41 step 8)
    assert res.lower_bound <= res.upper_bound
