...
def make_sources(mode, seed, n_sources=None, scenario="random",
    ...
    t10 新增可选参数 directional_share（默认 None ⇒ 行为与历史完全一致，
    RNG 序列不变）：仅 mode=="Q4" 且显式给定时，按 round(n*share) 抽取
    定向源个数（rng.shuffle 确定性分配到各源），用于把 Q4 案例口径校准到
    官方演练初步观测（定向占比 76.9%，40/52，4 局，Wald95 CI≈[65%,88%]）。
    """
    import random
    ...
    q4_flags = None
    if mode == "Q4" and directional_share is not None:
        k = int(round(n_sources * float(directional_share)))
        k = max(0, min(n_sources, k))
        q4_flags = [True] * k + [False] * (n_sources - k)
        rng.shuffle(q4_flags)
    sources = []
    for ch, (x, y) in zip(channels, positions):
        ...
        if mode == "Q3":
            ...
            if q4_flags is not None:
                directional = q4_flags.pop(0)
            elif scenario == "mixed":
...
class SyntheticSimulator:
    def __init__(self, mode, seed, n_sources=None, scenario="random",
        ...
        # t10 校准（附件2 §6.2）：/enter 的 remaining_real_duration_s 应可注入；
        # 默认 None ⇒ 维持旧行为（恒 1200）。官方在晚启动时会返回 <1200。
        self._remaining_override = (None if remaining_real_duration_s is None
                                    else float(remaining_real_duration_s))
        # 会话状态
        self.entered = False
        self.exited = False
        self.robot_id = None
    ...
    def handle(self, endpoint, payload):
        ...
        def err400(msg):
            # t10 校准（附件2 §5.3）：所有能形成 HTTP 响应的错误都返回 JSON，
            # 至少包含 accepted / real_timestamp_ms / virtual_time_s。
            return 400, {"error": msg, "accepted": False,
                         "real_timestamp_ms": now_ms, "virtual_time_s": 0}
        ...
        if rid in self._idempotency:
            ...
            # t10 校准（附件2 §5.3）：错误响应也带三公共字段。
            return 409, {"error": "request_id conflict", "accepted": False,
                         "real_timestamp_ms": now_ms, "virtual_time_s": 0}
        ...
        # t10 校准（附件2 §5.3）：仅 accepted=true 的动作占用 request_id；
        # accepted=false 的 200（结构/标识拒绝）不占用，修正后可复用同 id。
    ...
    def _do_enter(self, payload, now_ms, reject200):
        ...
        self._last_measure_channel = None
        self._t0 = time.time()
        remaining = (self._remaining_override if self._remaining_override
                     is not None else MAX_REAL_DURATION_S)
    ...
    def _do_measure(self, pos, channel, now_ms, reject200):
        ...
        dt += C.MEASURE_TIME
        # t10 校准（附件2 §1.4/§4.3）：测向机初始频道为 1，本次合法 /measure
        # 的 channel 与"当前频道"（首测前=1，其后=上一次合法 /measure 频道）
        # 不同即 +1s。旧实现在首测时跳过换频，属语义偏差。
        last_ch = self._last_measure_channel \
            if self._last_measure_channel is not None else 1
        if last_ch != int(channel):
            dt += C.SWITCH_TIME
    ...
    def _do_exit(self, now_ms, reject200):
        if not self._open():
            return reject200()
        self.exited = True
        # t10 校准（附件2 §9.2）：exit_reason 固定为 "user_exit"。
        return 200, {"accepted": True, "real_timestamp_ms": now_ms,
                     "virtual_time_s": self._vt(),
...
class _SimHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        sim = self.server.sim
        # t10 校准（附件2 §5.1）：路径必须精确为 /enter /measure /clear /exit，
        # 不接受尾随斜线与查询参数（官方 404）。未读请求体即应答，必须关闭
        # 连接，避免污染后续请求解析。
        path = self.path.split("?", 1)[0]
        if path != self.path or path not in protocol.ENDPOINTS:
        ...
        if length > protocol.MAX_REQUEST_BYTES:
            # t10 校准（附件2 §5.3）：请求体超过 65536 字节 ⇒ HTTP 413。
            # 未读请求体即应答，关闭连接避免污染后续请求解析。
            self.close_connection = True
