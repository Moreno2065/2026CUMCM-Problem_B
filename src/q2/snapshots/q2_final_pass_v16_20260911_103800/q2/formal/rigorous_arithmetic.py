...
def _decimal_outward(ball, digits: int, toward_negative: bool) -> str:
    """Decimal string of an arb endpoint ball, bumped one ulp outward."""
    ...
    with localcontext() as ctx:
        ctx.prec = digits + 40
        d = Decimal(s)
        exp = d.adjusted() - (digits + 8) + 1
        quantum = Decimal(1).scaleb(exp)
        bump = quantum * 3
        if toward_negative:
            out = (d - bump).quantize(quantum, rounding=ROUND_DOWN)
        else:
            out = (d + bump).quantize(quantum, rounding=ROUND_UP)
    return format(out, "f")
