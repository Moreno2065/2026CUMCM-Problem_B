...
def derive_search_bounds(a1: A1Region, crec: CrecRegion) -> SearchBounds:
    """Intersect axis boxes of mandatory Crec witness disks."""
    ...
    bounds = SearchBounds(
        ...
        derivation="intersection_of_crec_witness_disk_boxes",
    )
...
def _search_outer_impl(
    ...
    def evaluate(point: Point2, stage: str) -> Q2PointResult:
        return evaluate_many((point,), stage)[0]
    ...
    for direction in _directions():
        point = Point2(
            incumbent.S2.x + direction.x * final_step,
            incumbent.S2.y + direction.y * final_step,
        )
        if not bounds.contains(point):
            continue
        previous = incumbent
        result = evaluate(point, "final_neighbor_check")
        if previous.Q is not None and _better(result, previous.result):
            no_better = False
...
def _result_key(result: Q2PointResult) -> tuple[float, float, float]:
    return (math.inf if result.Q is None else result.Q, result.S2.x, result.S2.y)
