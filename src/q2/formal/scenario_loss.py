...
class Scenario:
    ...
    def e_interval(self) -> Ivl:
        """Rigorous radians enclosure of the stored degree error.

        Converting with ``math.radians`` first would freeze a rounded binary
        radian value and can exclude the exact degree-to-radian conversion.
        """
        return Ivl.from_float(self.e_deg) * Ivl.pi() / Ivl.from_int(180)
