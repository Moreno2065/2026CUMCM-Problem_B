# absorbed/q3_v5 — 源：D:\CUMCM2026\src\Q3_Q4_V3\code\src\geometry.py (175 行)
# 唯一改动：文件名 geometry.py -> geometry_src.py。目标包运行时会把包根与 baseline\code 插到
# sys.path 最前（run.py:29-32、runtime.py:15-18），且 runtime.py:24 使用 "from geometry import
# constants as C"，同名会静默解析到别的模块。本文件自身无包内 import，正文 175 行逐行未改。
# 详见同目录 MANIFEST.md。
